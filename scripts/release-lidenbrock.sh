#!/usr/bin/env bash
# Release script for packages/lidenbrock.
#
# Usage:
#   scripts/release-lidenbrock.sh             # build + smoke-install only
#   scripts/release-lidenbrock.sh --testpypi  # build + upload to TestPyPI
#   scripts/release-lidenbrock.sh --pypi      # build + upload to PyPI (prod)
#
# Pre-flight (manual):
#   1. Bump src/lidenbrock/__init__.py::__version__
...
# .github/workflows/publish-lidenbrock.yml for the CI path.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PKG_DIR="${REPO_ROOT}/packages/lidenbrock"
TARGET=""
...
init = pathlib.Path('src/lidenbrock/__init__.py').read_text()
...
if not m:
    sys.exit('__version__ not found in src/lidenbrock/__init__.py')
print(m.group(1))
")
echo "==> Releasing lidenbrock ${VERSION}"
...
# with .github/workflows/{ci,publish-lidenbrock}.yml — see roadmap
...
INSTALLED_VERSION=$("${SMOKE_VENV}/bin/python" -c "import lidenbrock; print(lidenbrock.__version__)")
...
if [[ "${TARGET}" == "testpypi" ]]; then
    INDEX_JSON_URL="https://test.pypi.org/pypi/lidenbrock/json"
else
    INDEX_JSON_URL="https://pypi.org/pypi/lidenbrock/json"
fi
INDEX_TMP="$(mktemp)"
...
"; then
            echo "ERROR: lidenbrock ${VERSION} is already on ${TARGET}." >&2
            echo "       Bump src/lidenbrock/__init__.py::__version__ and add a CHANGELOG entry." >&2
            rm -f "${INDEX_TMP}"
            exit 1
        fi
        ...
        ;;
    404)
        echo "==> Index check: lidenbrock not yet published on ${TARGET}, first release."
        ;;
    000)
...
if [[ "${TARGET}" == "testpypi" ]]; then
    ...
    echo "==> Verify at https://test.pypi.org/project/lidenbrock/${VERSION}/"
else
    echo "==> Uploading to PyPI (production)"
    read -r -p "Confirm production upload of lidenbrock ${VERSION}? [y/N] " confirm
    if [[ "${confirm}" != "y" && "${confirm}" != "Y" ]]; then
        echo "Aborted."
        exit 1
    fi
    python -m twine upload dist/*
    echo "==> Verify at https://pypi.org/project/lidenbrock/${VERSION}/"
fi
