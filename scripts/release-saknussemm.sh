#!/usr/bin/env bash
# Release script for packages/saknussemm.
#
# Usage:
#   scripts/release-saknussemm.sh             # build + smoke-install only
#   scripts/release-saknussemm.sh --testpypi  # build + upload to TestPyPI
#   scripts/release-saknussemm.sh --pypi      # build + upload to PyPI (prod)
#
# Pre-flight (manual):
#   1. Bump src/saknussemm/__init__.py::__version__
...
# .github/workflows/publish-saknussemm.yml for the CI path.
...
if not m:
    sys.exit('__version__ not found in src/saknussemm/__init__.py')
print(m.group(1))
")
echo "==> Releasing saknussemm ${VERSION}"
...
# with .github/workflows/{ci,publish-saknussemm}.yml — see roadmap
...
INSTALLED_VERSION=$("${SMOKE_VENV}/bin/python" -c "import saknussemm; print(saknussemm.__version__)")
...
if [[ "${TARGET}" == "testpypi" ]]; then
    INDEX_JSON_URL="https://test.pypi.org/pypi/saknussemm/json"
else
    INDEX_JSON_URL="https://pypi.org/pypi/saknussemm/json"
fi
INDEX_TMP="$(mktemp)"
...
"; then
            echo "ERROR: saknussemm ${VERSION} is already on ${TARGET}." >&2
            echo "       Bump src/saknussemm/__init__.py::__version__ and add a CHANGELOG entry." >&2
            rm -f "${INDEX_TMP}"
            exit 1
        fi
        ...
        ;;
    404)
        echo "==> Index check: saknussemm not yet published on ${TARGET}, first release."
        ;;
    000)
...
if [[ "${TARGET}" == "testpypi" ]]; then
    ...
    echo "==> Verify at https://test.pypi.org/project/saknussemm/${VERSION}/"
else
    echo "==> Uploading to PyPI (production)"
    read -r -p "Confirm production upload of saknussemm ${VERSION}? [y/N] " confirm
    if [[ "${confirm}" != "y" && "${confirm}" != "Y" ]]; then
        echo "Aborted."
        exit 1
    fi
    python -m twine upload dist/*
    echo "==> Verify at https://pypi.org/project/saknussemm/${VERSION}/"
fi
