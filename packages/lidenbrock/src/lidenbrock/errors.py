"""lidenbrock error hierarchy (SPECS_LIB_V2 §8.4).

A single root, ``LidenbrockError``, sits above every error the library
raises so consumers can ``except LidenbrockError`` once::
...
Naming: the root is ``LidenbrockError`` — named for the LIBRARY,
...
class LidenbrockError(Exception):
    """Base class for every error raised by lidenbrock (§8.4)."""

    code: ClassVar[str] = "correction_error"
    retryable: ClassVar[bool] = False


class ParseError(LidenbrockError, ValueError):
...
class DuplicateIdError(ParseError):
    ...
    Inherits :class:`ParseError` (hence ``LidenbrockError`` and
...
class ProposalValidationError(LidenbrockError, ValueError):
    """A producer (LLM / rules / VLM) response failed validation.

    Raised by :func:`lidenbrock.core.validator.validate_llm_response`
...
class ProviderError(LidenbrockError):
    """Base for failures reported by a producer's underlying provider.

    The concrete classes live in :mod:`lidenbrock.core.protocols` next to
    the ``BaseProvider`` contract that raises them; this base anchors them
    under the single ``LidenbrockError`` root. Carries the originating
...
class ConfigurationError(LidenbrockError):
...
class ProjectionError(LidenbrockError):
...
class CorrectionAborted(LidenbrockError):
...
CorrectionError = LidenbrockError
