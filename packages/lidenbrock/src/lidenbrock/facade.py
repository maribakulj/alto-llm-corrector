...
::
    ...
    document = lidenbrock.load("page.xml")            # format by namespace
    result = await lidenbrock.correct(document, producer=producer)
    result.write("out/")
...
:class:`~lidenbrock.core.pipeline.CorrectionPipeline` around any
:class:`~lidenbrock.core.protocols.EditProducer` with a no-op observer.
...
LAZILY from the top level: ``import lidenbrock`` alone stays pure.
"""
...
from lidenbrock.core.protocols import EditProducer
from lidenbrock.core.schemas import DocumentManifest, PageImage
...
def load(*paths: str | Path) -> LoadedDocument:
    ...
    """
    if not paths:
        raise ParseError("lidenbrock.load() needs at least one file path")
...
def correct_sync(
    ...
    *,
    ...
    ``await lidenbrock.correct(...)`` there.
    """
    pipeline = CorrectionPipeline(producer=producer, observer=_NullObserver())
