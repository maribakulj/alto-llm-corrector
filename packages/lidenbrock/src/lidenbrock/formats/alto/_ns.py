...
the hardened ``make_safe_parser`` — live in :mod:`lidenbrock.formats._xml`
and are re-exported here under their historical private names so ALTO call
sites (``from lidenbrock.formats.alto._ns import _tag, …``) are unchanged.
...
def _int_attr(el: etree._Element, name: str, default: int = 0) -> int:
    ...
    parse policy lives in :func:`lidenbrock.core._parse.parse_int_tolerant`,
...
# from ``lidenbrock.formats._xml`` — the format-neutral home — so older
...
# hardened parser is now ``from lidenbrock.formats._xml import make_safe_parser``.
