...
(``corrigenda[qe]``: onnxruntime, tokenizers, a 545 MB model) and
``integrations/vision.py`` (``corrigenda[vision]``: Pillow). Both predate
...
Three rules, and the first is the one with teeth:
  ...
  1. no module reachable from ``import corrigenda`` imports a gated
...
_GATED = ("corrigenda.integrations.qe", "corrigenda.integrations.vision")
...
def test_importing_the_package_loads_no_gated_module() -> None:
    """``import corrigenda`` is the base install path."""
    loaded = _import_probe("import corrigenda")
    assert not loaded, (
        f"importing corrigenda pulled in {sorted(loaded)} — those are behind "
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("corrigenda.core.quality", "HeuristicQEScorer"),
        ("corrigenda.core.quality", "RoutingPolicy"),
        ("corrigenda.core.schemas", "ConfidencePolicy"),
        # scripts/vision_benchmark.py, scripts/ocr_corpus.py
        ("corrigenda.core.schemas", "ImageAsset"),
        ("corrigenda.core.schemas", "ImageTransform"),
        ("corrigenda.core.schemas", "ModelCapabilities"),
    ],
)
