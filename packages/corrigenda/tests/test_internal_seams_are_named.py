...
_SEAMS: dict[str, tuple[str, str]] = {
    ...
    "corrigenda.__version__": ("surface", "a dunder; the provenance test reads it"),
    "corrigenda._LAZY": (
        "surface",
    ...
    "corrigenda.core.batching._split_for_image_cap": (
        "value",
        "returns the chunk/producer pairs; the test asserts no unit is split",
    ),
    "corrigenda.core.hyphenation._part1_text_migrated": (
        "value",
        "a predicate over two strings and a config",
    ),
    "corrigenda.core.hyphenation._part2_boundary_word_diverged": (
        "value",
        "a predicate over two strings and a config",
    ),
    "corrigenda.core.planner._unit_reach": (
        "value",
    ...
    "corrigenda.formats.alto.rewriter._subs_need_update": (
        "value",
        "the route predicate; its agreement with _apply_subs is a fixed point "
        "no run output exposes",
    ),
    "corrigenda.formats.alto.rewriter._tokenize": ("value", "text in, tokens out"),
    "corrigenda.formats.page._ns._namespace_year": (
        "value",
        "namespace in, schema year out",
    ),
    "corrigenda.formats.page.parser._assign_hyphen_roles": (
        "value",
        "assigns roles onto lines the TEST built — no run, no traces",
    ),
    "corrigenda.formats.page.parser._regions_in_reading_order": (
        "value",
        "returns the ordered regions; the conservative fallback to document "
        "order is invisible from outside",
    ),
    "corrigenda.formats.validation._schema_for": (
        "value",
        "namespace in, compiled schema out",
    ),
    "corrigenda.integrations.qe._deglyph": ("value", "text in, text out"),
    "corrigenda.integrations.qe._deglyph_with_map": (
        "value",
    ...
    for path in _test_modules():
        ...
        for node in ast.walk(tree):
            if not isinstance(node, ast.ImportFrom):
                continue
            if not node.module or not node.module.startswith("corrigenda"):
                continue
...
def _module_file(dotted: str) -> Path:
    relative = dotted[len("corrigenda") :].strip(".").replace(".", "/")
...
def test_the_aliases_all_come_from_one_place() -> None:
    ...
    assert origins == {"corrigenda.formats._xml.detect_namespace"}, (
