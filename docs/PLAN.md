...
| S2 | Scinder `core/pipeline.py` en composants nommés — préflight, planification, routage, exécution de chunk, validation, acceptation, réconciliation d'unités, projection, assemblage du rapport. Le pipeline public **orchestre**, il ne réimplémente pas | **3244 → 1686** (−48 %), 12 modules nommés, 7 tranches (2026-07-28). Reste le **contrôle d'exécution** et lui seul : `_run_impl` 311, `_attempt_chunk` 221, `_run_chunk` 158, `_process_page` 157 | fichier principal < 800 l., aucune méthode > 100 l., assemblage du rapport indépendant du contrôle d'exécution |
