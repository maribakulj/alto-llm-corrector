...
| `H-2` | **Résorber les modules squelettes.** Critère : sous 80 lignes de code **et** un seul appelant interne **et** aucune capacité propre à nommer. Mesuré le 2026-09-02 — quatre modules sur les seize remplissent les deux premières conditions, et le troisième point est ce qui reste à trancher pour chacun : `core/indexing.py` (47 l., appelé par `pipeline` seul) ; `core/finalize.py` (55 l., `pipeline` seul — et **trois de ses quatre passes vivent déjà dans `acceptance.py`**, ce qui en fait le rapprochement le plus cohésif de la liste) ; `core/page_alignment.py` (51 l., `producers/page_llm.py` seul) ; `core/batching.py` (72 l., `routing` seul). `workspace.py` (10 l., 4 appelants) part avec `H-1` si le chemin fusionné rend ses trois index naturellement. **Ne partent pas** : `redaction.py` (couture de sécurité), `fidelity.py` et `projection.py` (vocabulaire public du rapport). **Attention au cliquet** : fusionner dans `pipeline` se heurte à `_MODULE_BUDGET`, et grossir l'orchestrateur contredirait `S2` — donc `indexing` et `finalize` ne remontent pas vers `pipeline`, ils descendent vers le module dont ils portent le travail | −3 à −4 fichiers |
| ~~`H-3`~~ | **Fait le 2026-09-02, et la raison inscrite ici était fausse.** Elle disait que `tests/decision/test_finalize_pass_order.py` rendait déjà le jeton redondant. Il ne le rendait pas : ses deux scénarios appellent les passes **eux-mêmes**, avec `order=None`, donc ils démontraient que l'ordre change le texte livré sans rien affirmer sur celui de `_finalize_document`. Ce que le jeton gardait vraiment, seul, c'était la fonction de production — avec un test `ast` qui relisait la source pour vérifier qu'il y était bien passé. La preuve manquante a donc été construite d'abord : `test_finalize_document_delivers_the_canonical_outcome` fait tourner la vraie `_finalize_document` sur le document où les passes interagissent et épingle l'issue. **Vérifié par mutation, deux fois** : jeton neutralisé, la permutation fait rougir l'assertion (`L2` livre `LE CHAT GRIS TRES` au lieu de revenir à sa source) ; jeton retiré, elle rougit encore. Le jeton, son helper `_entering`, les quatre paramètres `order=`, le test `ast` et les trois tests qui n'existaient que pour lui s'en vont | −124 lignes nettes (−44 de code, −35 de prose), −1 concept, −1 test structurel remplacé par un test comportemental. **Ratio de prose inchangé à 0,860** — cet item retire un concept, pas de la densité |
...
| `H-5` | **Supprimer les six branches mortes** — `claude/repo-technical-audit-1afr82`, `claude/a2b-a-refused-edit-is-visible`, `ligne-source-vide`, `phase4-brique1-integration`, `phase4-complete`, `promesses-etat-perime`. **Vérifié fichier par fichier le 2026-09-02** : aucune ne porte une seule ligne que `main` n'ait déjà. La seule qui en portait — cinq lignes de `docs/AUTOPILOT.md` disant « brique 1 faite, restent quatre briques » — est supersédée par l'état « les cinq briques, FAITE » que `main` porte. `couverture-tranchee` **reste** : c'est la PR `#162`, ouverte. **Demande le mainteneur** : les identifiants de session poussent sur une branche et ne suppriment pas de ref (403), et le serveur MCP n'expose pas l'opération | le dépôt cesse de suggérer six chantiers ouverts |
...
| `G4` | **La provenance ne couvre pas tout ce qui change les octets — arbitrage, pas correctif.** `config_fingerprint()` agrège cinq politiques ; `routing_policy`, `review_policy`, `qe_scorer` et `confidence_policy` n'y sont pas. Or `routing.py` fait du texte OCR le texte final d'une ligne `SKIP` : deux runs aux seuils différents livrent des octets différents **sous la même empreinte**. La docstring de `RoutingPolicy` connaît le sujet et écrit que la politique n'y entre « pas tant qu'un run n'a pas effectivement sauté ou escaladé une ligne » — mais aucun code ne l'ajoute jamais. **Portée réelle : les deux bornes valent `None` par défaut et le routage exige un scorer**, donc c'est un défaut d'un chemin expérimental, pas du chemin nominal — d'où `1.0` et non `0.10`. Les deux sorties sont mauvaises telles quelles, ce qui est la raison de l'arbitrage : **inconditionnel** change toute empreinte déjà émise et casse la reproductibilité des runs passés ; **conditionnel** rend l'empreinte non recalculable à partir de la seule configuration, ce que sa propre docstring promet (« a consumer holding the same policy objects can recompute and verify it »). Une troisième voie existe — un champ de provenance distinct pour les politiques actives, à côté de l'empreinte plutôt que dedans — et elle ne casse ni l'une ni l'autre. Relevé le 2026-09-01, non tranché |
