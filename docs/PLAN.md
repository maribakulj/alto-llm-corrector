...
| S2 | Scinder `core/pipeline.py` en composants nommés — préflight, planification, routage, exécution de chunk, validation, acceptation, réconciliation d'unités, projection, assemblage du rapport. Le pipeline public **orchestre**, il ne réimplémente pas | **3244 → 965** (−70 %), 18 modules nommés, 11 tranches (2026-08-01). `_run_impl` 264 → 98, `_attempt_chunk` 221 → sorti (`core/attempt.py`), `_process_page` 165 → 87, `_run_chunk` 158 → 104. Assemblage du rapport indépendant du contrôle d'exécution : **atteint**. **Cible « aucune fonction > 100 l. » mesurée sur tout `core/`, pas seulement sur `pipeline.py` : 9 fonctions restent au-dessus, dont 3 que des tranches antérieures ont sorties de `pipeline.py` à leur taille d'origine** (`_render_outputs` 140, `_reconcile_chunk_hyphens` 158, `_route_and_filter_chunks` 110). Épinglé et non croissant par `tests/test_orchestrator_budget.py`. Candidat identifié pour la suite : le paquet `(ctx, chunk, chunk_lines, line_by_id, traces, cross_page_partners)` voyage ensemble à travers 5 fonctions — le réduire raccourcirait `_run_chunk`, `_descend_granularity` et `core/outcome.py` d'un coup | fichier principal < 800 l., aucune fonction de `core/` > 100 l., assemblage du rapport indépendant du contrôle d'exécution |
