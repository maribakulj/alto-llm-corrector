...
| S2 | Scinder `core/pipeline.py` en composants nommés — préflight, planification, routage, exécution de chunk, validation, acceptation, réconciliation d'unités, projection, assemblage du rapport. Le pipeline public **orchestre**, il ne réimplémente pas | **3244 → 1443** (−56 %), 15 modules nommés, 9 tranches (2026-08-01). `_run_impl` 264 → 93 et l'assemblage du rapport est sorti du moteur ; la cible est désormais **tenue par un test** (`tests/test_orchestrator_budget.py` : budget-cliquet + liste explicite des méthodes hors cible). Reste : `_attempt_chunk` 221, `_process_page` 165, `_run_chunk` 158 | fichier principal < 800 l., aucune méthode > 100 l., assemblage du rapport indépendant du contrôle d'exécution |
