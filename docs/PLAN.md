...
| S2 | Scinder `core/pipeline.py` en composants nommés — préflight, planification, routage, exécution de chunk, validation, acceptation, réconciliation d'unités, projection, assemblage du rapport. Le pipeline public **orchestre**, il ne réimplémente pas | **3244 → 2105** (−35 %), 10 modules nommés extraits en 4 tranches (2026-07-28) ; reste `_run_impl` 311, `_attempt_chunk` 221, `_run_chunk`/`_process_page`/`_reconcile_chunk_hyphens` ~157 | fichier principal < 800 l., aucune méthode > 100 l., assemblage du rapport indépendant du contrôle d'exécution |
