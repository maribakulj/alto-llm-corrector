...
| S2 | Scinder `core/pipeline.py` en composants nommés — préflight, planification, routage, exécution de chunk, validation, acceptation, réconciliation d'unités, projection, assemblage du rapport. Le pipeline public **orchestre**, il ne réimplémente pas | **3244 → 568** (−82 %), 19 modules nommés, 12 tranches (2026-08-01). **Deux des trois cibles atteintes** : fichier principal **568 < 800**, assemblage du rapport indépendant du contrôle d'exécution. La boucle interne est `core/driver.py` (`PageDriver`, configuration immuable liée une fois par run) ; `CorrectionPipeline` ne fait plus que construire, préflighter, piloter, finaliser, rendre, rapporter. **Troisième cible non atteinte** : mesurée sur tout `core/` et non sur le seul `pipeline.py`, **11** fonctions dépassent 100 lignes : **4 sont passées par `S2`** (`_reconcile_chunk_hyphens` 158, `_render_outputs` 140, `_route_and_filter_chunks` 110 — sorties de `pipeline.py` à leur taille d’origine — et `driver.py::_run_chunk` 104) et **7 préexistent entièrement à `S2`** (`validator` 149, `pairing` 119, `editing` 114 et 103, `hyphenation` 110 et 101, `acceptance` 107). Épinglé et non croissant par `tests/test_orchestrator_budget.py`. Candidat pour la suite : le paquet `(ctx, chunk, chunk_lines, line_by_id, traces, cross_page_partners)` voyage à travers 5 fonctions du driver | fichier principal < 800 l., aucune fonction de `core/` > 100 l., assemblage du rapport indépendant du contrôle d'exécution |
