...
| S2 | Scinder `core/pipeline.py` en composants nommés — préflight, planification, routage, exécution de chunk, validation, acceptation, réconciliation d'unités, projection, assemblage du rapport. Le pipeline public **orchestre**, il ne réimplémente pas | **3244 → 1112** (−66 %), 17 modules nommés, 10 tranches (2026-08-01). `_run_impl` 264 → 98, `_attempt_chunk` 221 → sorti du moteur (`core/attempt.py`, découpé, rien > 100 l.), `_run_chunk` 158 → 109. Assemblage du rapport indépendant du contrôle d'exécution : **atteint**. Reste dans le fichier : `_process_page` 165. **Cible « aucune méthode > 100 l. » mesurée sur tout `core/`, pas seulement sur `pipeline.py` : 9 fonctions restent au-dessus, dont 3 que des tranches antérieures ont sorties de `pipeline.py` à leur taille d'origine** (`_render_outputs` 140, `_reconcile_chunk_hyphens` 158, `_route_and_filter_chunks` 110). Le tout est épinglé et non croissant par `tests/test_orchestrator_budget.py` | fichier principal < 800 l., aucune fonction de `core/` > 100 l., assemblage du rapport indépendant du contrôle d'exécution |
