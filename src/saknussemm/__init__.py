"""saknussemm — structure-safe post-OCR correction of heritage transcriptions.

Sub-packages (§3 tree):

- :mod:`saknussemm.core` — pure algorithms: schemas, guards, hyphenation
...
- :mod:`saknussemm.formats` — concrete transcription formats. Today
  :mod:`saknussemm.formats.alto` (hardened parser + 4-path rewriter);
  PAGE XML plugs in the same seam.
- :mod:`saknussemm.producers` — edition-producer surfaces (the LLM
...
:mod:`saknussemm` from a core-only consumer never loads lxml.
"""

from typing import TYPE_CHECKING, Any

from saknussemm.core.decisions import (
    DecisionSet,
    LineDecision,
)
...
from saknussemm.core.pipeline import (
    CorrectionPipeline,
    CorrectionResult,
    sanitize_error,
)
...
if TYPE_CHECKING:  # typed view of the lazy symbols below
    from saknussemm.facade import LoadedDocument as LoadedDocument
    from saknussemm.facade import correct as correct
    from saknussemm.facade import correct_sync as correct_sync
    from saknussemm.facade import load as load
...
# agree):
#   1. either an eager ``from saknussemm.core... import`` above (pure-core
#      symbols, no lxml) OR the ``_LAZY`` map below (format/producer symbols
#      that must stay lazy so ``import saknussemm`` never loads lxml);
...
#: on first attribute access — `import saknussemm` alone stays pure.
_LAZY: dict[str, str] = {
    "LoadedDocument": "saknussemm.facade",
    "correct": "saknussemm.facade",
    "correct_sync": "saknussemm.facade",
    "load": "saknussemm.facade",
}
...
def __getattr__(name: str) -> Any:
    ...
    if module_name is None:
        raise AttributeError(f"module 'saknussemm' has no attribute {name!r}")
    import importlib
