...
the hardened ``make_safe_parser`` — live in :mod:`saknussemm.formats._xml`
and are re-exported here under their historical private names so ALTO call
sites (``from saknussemm.formats.alto._ns import _tag, …``) are unchanged.
...
def _int_attr(el: etree._Element, name: str, default: int = 0) -> int:
    ...
    parse policy lives in :func:`saknussemm.core._parse.parse_int_tolerant`,
...
# from ``saknussemm.formats._xml`` — the format-neutral home — so older
...
# hardened parser is now ``from saknussemm.formats._xml import make_safe_parser``.
