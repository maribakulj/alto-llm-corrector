...
members in reading order, keyed by :class:`~saknussemm.core.identity.LineRef`.
...
from saknussemm.core.schemas import HyphenRole, HyphenSplit, LineManifest
...
def split_forward_link(tail: LineManifest, head: LineManifest) -> HyphenSplit:
    ...
    * ``head`` BOTH → PART1 (its own forward link/subs migrate into the
    ...
    :class:`~saknussemm.core.schemas.ChunkPlan` — is how a consumer
