...
members in reading order, keyed by :class:`~lidenbrock.core.identity.LineRef`.
...
from lidenbrock.core.pairing import forward_partner_id, forward_ref, pair_ref
from lidenbrock.core.schemas import HyphenRole, HyphenSplit, LineManifest
...
def split_forward_link(tail: LineManifest, head: LineManifest) -> HyphenSplit:
    ...
    * ``head`` BOTH → PART1 (its own forward link/subs migrate into the
    ...
    :class:`~lidenbrock.core.schemas.ChunkPlan` — is how a consumer
