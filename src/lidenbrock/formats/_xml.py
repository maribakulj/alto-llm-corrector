...
from lidenbrock.errors import LidenbrockError, ParseError
...
def make_safe_parser(encoding: str | None = None) -> etree.XMLParser:
    ...
    ``packages/lidenbrock/tests/test_xml_security.py`` trips on any call
...
def classified_parse_errors(source_name: str) -> Iterator[None]:
    """§8.4 — a parser may only raise classified :class:`LidenbrockError`s.
    ...
    :class:`DuplicateIdError`, any :class:`LidenbrockError` — pass through
